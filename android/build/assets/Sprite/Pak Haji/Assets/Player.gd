...
@export var jump_count = 2 # (int, 1, 100)
...
var DEFAULT_GRAVITY = Vector2(0,1).rotated(deg_to_rad(0)) #TODO Make Const
...
func _physics_process(delta):
	...
	set_velocity(velocity.rotated(-grav_rot))
	# TODOConverter40 looks that snap in Godot 4.0 is float, not vector like in Godot 3 - previous value `gravity_normal*snap`
	set_up_direction(-gravity_normal)
	set_floor_stop_on_slope_enabled(true)
	set_max_slides(4)
	set_floor_max_angle(deg_to_rad(80))
	move_and_slide()
	velocity = velocity.rotated(grav_rot)
...
func calculate_gravity_normal():
	...
	if gravity_influence.is_empty() || !can_gravity:
		n_g = DEFAULT_GRAVITY
	else:
	#	var mk = ""
...
func calculate_sprite():
	...
	var n_rot = rad_to_deg(gravity_normal.angle()) - 90
...
func get_state():
	...
	if last_state == STATES.Glide && Input.is_action_pressed("jump") && !Input.is_action_just_pressed("jump") && !wind_influence.is_empty() && can_glide:
